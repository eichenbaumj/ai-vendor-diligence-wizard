...
|---|---|---|---|
| **1. parse** | Ingestion first for pdf/url submissions (`ingest-pdf.ts` text-layer extraction with tiny-font/off-page detection; `ingest-url.ts` guarded fetch + hidden-HTML detection on the raw page), then deterministic forensics (`forensics.ts`: invisible-Unicode strip, instruction-pattern screen, PII backstop), then a quarantined extraction call turns the pitch into typed claims. S1b: the vendor's public site (stated by the pitch, submitted, or found by a 2-search discovery lookup for name-only runs) is fetched under `ingest-site.ts` bounds (≤5 nav-level pages, hidden text stripped pre-extraction), extracted by a second quarantined S1 call, and merged under `extract-merge.ts` provenance rules: site text creates things to check, never identity, never registry-query names, never ADV-ceiling findings, never absence-based findings | Claude Haiku 4.5, no tools, strict JSON schema output, temperature 0 (methodology 1.7: the extractor runs at its most repeatable setting) | `PitchExtract` |
| **2. registry** | Deterministic fan-out against public registries: EDGAR, the five open-data state registries, RDAP, Wayback CDX, crt.sh, SAM entity + exclusions, USAspending, FedRAMP feed, GovRAMP list, TX-RAMP (when Texas), Sourcewell, GitHub org, DNS/MX. Each check logs query, timestamp, status, and evidence URL. Name-only runs get a second pass (S2b) after research: if the citations nominate the vendor's website under the rules in `domain-inference.ts` (Channel-A citations only, class 3 hosts, name-covering registrable domain, ≥2 distinct URLs), the domain-hygiene lanes run against the inferred domain, labeled as inferred and never counted toward identity resolution | None (plain code) | `RegistryLedger` |
| **3. research** | Agentic web research: customer traces on .gov sites, leadership corroboration, case-study cross-existence, AI-inventory presence. Citations required on everything; application code classifies each cited domain (`domain-classes.ts`) | Claude Sonnet 5 with `web_search_20250305` and `web_fetch_20250910` (12 searches / 6 fetches standard, 20 / 8 when the pitch names 4+ customers; citations on, content capped at 15k tokens), content-farm domains blocked; the work arrives in bounded `pause_turn` cycles, so completed cycles survive even when a later one hits the stage deadline. Deep mode: `evaluate` checkpoints its typed state and hands off to `deep-research`, which starts a fresh 400s clock and runs four objective-scoped lanes in parallel (15 searches / 6 fetches each, one `DEEP_MODE` config object), merges citations, and runs the same shared pipeline tail (`pipeline-tail.ts`) | `ResearchOutput` |
| **4. packs** | Use-case classification, high-impact escalation, state-obligation mapping; selects the sector packs | Claude Haiku 4.5 with strict schema output; a published signal-lexicon fallback in code covers a failed or empty model pass, and the eligibility lexicon can only add scrutiny | `SectorContext` |
| **5. synthesis** | Composes ledger rows and narrative (`composeReport` in `pipeline-tail.ts`, a pure seam); computes the verdict tier in plain code (`tier.ts`) from typed inputs; selects the question pack in plain code (`questions.ts`) from templates and published pack metadata. Green flags and the identity sentence are code templates over the credited record. Every model sentence passes the language lint (`lint.ts`) and the synthesis guard (`synthesis-guard.ts`): it may name only the vendor and the legal names the run credited; a sentence naming any other company is dropped and an empty surface falls back to a template | Claude Haiku 4.5 with strict JSON schema output (no green-flag field), plus non-LLM validators | `Report` |
| **6. review** | Optional wording pass over the assembled report JSON on adverse, contradicted, or adversarial-content reports (`applyReview`, a pure seam): it can tighten row notes and the summary, each adjustment logged with a fixed label. In code it can never touch the tier, remove a High or Critical row, a contradicted or official-record row, or the identity row; its replacement text passes the same lint and name screens; a pass that does not complete within its time allowance changes nothing | Recorded per-report in `report.review.model` | `Report.review` |
...
From the [cost model](./research/gap-no-per-evaluation-cost-model-an-explicit.md) (all rates verified August 2026), scaled to the current tool caps (12 searches / 6 fetches standard, 20 / 8 for many-customer pitches, 15k content tokens): observed evaluations run **$0.30 to $0.45** in API spend; extended-budget evaluations add roughly $0.15 to $0.20, and the all-caps-exhausted ceiling is roughly $2. The cost model's original figures assumed the larger launch caps. Composition at typical: the Sonnet 5 research stage is about $0.34 (a third of that is the $0.01-per-search fee), synthesis and extraction are a few cents each, registry checks are effectively free.
