...
Every adversarial-content screen in the pipeline is one-directional. The deterministic screens (pattern lists, invisible-character detection, hidden-HTML detection) fire findings that no later stage, human or model, can clear. The AI-based screen can add a finding the patterns missed; it cannot suppress one. A confirmed injection-class finding (ADV-01, ADV-02, ADV-03), which can exist only in material the submitter authored, caps the verdict at Tier 2, "Significant gaps." The cap only lowers, never raises. What counts as confirmed depends on the input: hidden text in a pasted document or uploaded PDF always caps, while hidden text on a submitted public web page caps only when it is instruction-like or carries a claim figure the visible page does not; ordinary web engineering (hidden menus, build timestamps, screen-reader labels) is reported informationally with the passage quoted, and never caps. Invisible characters cap on a payload class (tag or direction-control characters) or at volume, never on one stray zero-width character. ADV-04 reads the open web, which the vendor does not control, so it is reported prominently but never moves the tier; a tier that third parties could move by posting copied text would hand outsiders a lever over any vendor's verdict.
...
When a report shows an ADV finding, this is what it means. Each one quotes or links its evidence. The first three cap the verdict tier when classified as adversarial (hidden text on a submitted web page that is neither instruction-like nor claim-bearing is reported informationally instead, and small numbers of ubiquitous invisible characters strip silently); ADV-04 is reported without moving the tier.

| Code | Meaning | How it is detected |
|---|---|---|
| **ADV-01** | The submitted material contains substantive text hidden from human readers | Deterministic checks, live for web pages (hidden elements, zero-size or background-colored text, oversized comments) and for PDF text layers (text under 4 points, or placed off the page); the finding quotes the hidden passage. Color-matched PDF text and rendered-page comparison are planned; see limits below |
...
- **PDF hidden-text detection is partial.** ADV-01 now covers web pages and the PDF text layer: text smaller than 4 points and text placed outside the page fire a finding that quotes the hidden passage. What it does not yet catch: text colored to match the background, opacity tricks, and text absent from the rendered page (which needs rendering plus OCR comparison); those are designed and planned. A hidden layer that evades detection is still neutralized by the quarantine design above; it just is not surfaced as a finding.
