  ...
  try {
    ...
    if (awardCount > 0) {
      ...
      return {
        ...
        source,
        status: "hit",
        summary: `USAspending.gov shows ${awardCount} federal contract award${awardCount === 1 ? "" : "s"} to a recipient named ${matched.name} in the last five years, totaling about ${formatUsd(totalAmount)}${yearPart}. Federal payment records are strong evidence of real government work when the recipient is the same company; the identity check weighs the name match.`,
        evidence_url,
    ...
    return {
      ...
      source,
      status: "hit",
      summary: `USAspending.gov lists a recipient named ${matched.name}, but we did not find contract awards in the last five years. If this is the same company, ask the vendor when it last worked with a federal agency.`,
      evidence_url,
      confidence: matched.confidence,
      retrieved_at: nowIso(ctx),
      data,
