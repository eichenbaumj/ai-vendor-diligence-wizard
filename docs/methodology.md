...
**How it runs.** Automatic lookup.
**If adverse.** Claimed federal customers with zero footprint in the federal payment record is weighted High. No federal footprint with no federal claims is neutral. Verified payments are a strong green flag, but only on an exact name match: a recipient that merely resembles the vendor's name appears as a labeled candidate row ("name similarity only") with the record linked, and it never earns a green flag or moves any credit. Another company's awards must not decorate this vendor's report.
...
- **Match confidence is displayed, and it gates credit in both directions.** Every T1 match is labeled either "exact match" (legal name, identifier) or "name similarity only," and the label appears on the ledger row. Adverse findings on name-similarity matches are suppressed entirely: we show nothing rather than risk attaching a record to the wrong company. Favorable credit follows the same discipline: identity resolution, federal-award green flags, and the startup calibration bar count only exact matches, and a compliance-registry listing found under a similar name keeps its row and green flag but carries the similar-name label and the listed entity's actual name, so a namesake's record can never silently improve a verdict.
