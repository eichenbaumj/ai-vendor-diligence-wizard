  ...
  try {
    ...
    if (!res.ok) {
      return {
        ...
        summary: `The Internet Archive was unavailable, so we could not check this site's web history. This does not count against the vendor.`,
    ...
    return {
      ...
      source: SOURCE,
      status: "error",
      summary: `We could not reach the Internet Archive, so we could not check this site's web history. This does not count against the vendor.`,
      evidence_url: humanUrl,
      confidence: null,
      retrieved_at,
      data: null,
