...
| **3. research** | Agentic web research: customer traces on .gov sites, leadership corroboration, case-study cross-existence, AI-inventory presence. Citations required on everything; application code classifies each cited domain (`domain-classes.ts`) | Claude Sonnet 5 with `web_search_20250305` and `web_fetch_20250910` (12 searches / 6 fetches standard, 20 / 8 when the pitch names 4+ customers; citations on, content capped at 15k tokens), content-farm domains blocked; the work arrives in bounded `pause_turn` cycles, so completed cycles survive even when a later one hits the stage deadline | `ResearchOutput` |
