...
| **5. synthesis** | Composes ledger rows and narrative; computes the verdict tier in plain code (`tier.ts`) from typed inputs; selects the question pack in plain code (`questions.ts`) from templates and published pack metadata; runs the language lint (`lint.ts`), regenerating once on violation, falling back to neutral templates | Claude Haiku 4.5 with strict JSON schema output, plus non-LLM validators | `Report` |
