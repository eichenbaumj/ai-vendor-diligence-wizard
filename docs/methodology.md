...
The tool reads the vendor's own public website for every kind of submission, because most pitches leave it implicit and most readers do not know what to look for on it. What it reads: the homepage plus up to four high-value pages the homepage itself links (about, customers, security, and similar), fetched with strict size and time bounds. When the whole fetch fails and the run's clock allows, it is retried once from the start; a fetch that returned any pages is a result and is never re-run. Text hidden from human readers (invisible styling, oversized comments) is removed before any analysis sees it.
