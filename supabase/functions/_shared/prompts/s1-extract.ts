...
- claims: every checkable assertion, each with a verbatim quote (trim to the shortest span that carries the claim, but KEEP any qualifier, footnote, or asterisk text that limits the claim — "certified (pending approval)" must stay one quote). Type each one:
  ...
  For claims whose quote carries a verbatim number, also fill: amount (the number), unit (dollars, percent, hours, or count), period (annual, monthly, per_case, total, or unspecified), and basis_quote — the SHORTEST verbatim span elsewhere in the pitch that offers a denominator for the number, like "about 500 agents" or "40,000 permits a month", else null. Fill these only from numbers that appear verbatim; null everything when no number exists.
