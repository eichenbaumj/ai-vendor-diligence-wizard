...
**Methodology version 1.8** · Effective September 2, 2026 · Licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
...
Adverse findings carry a weight (Critical, High, Medium, Low, or Info) that feeds the verdict tier rules in Section 4. Passing checks are reported too, as green flags written by code from the checked record, each with its source and check date.
...
**Attribution: when a record counts as this vendor's.** A record that matches by name is credited to the vendor when a second detail ties it to them: an officer or registered agent named in the vendor's own materials or in independent coverage, an address the vendor uses, the vendor's web domain, the compliance feed's own product entry, or the complete legal name (corporate suffix included) that the buyer typed. A matching state supports credit on its own, but a state match alone can never arm a warning. The state may be claimed in the pitch, stated in the vendor's site addresses, or named in full (never abbreviated) by official or independent coverage that also mentions the vendor. A live record with exactly the vendor's name and no tie is credited only when three conditions hold. First, it is the only live record under that name across the registries searched. When records under different names compete, the name backed by more independent registries is credited and the others stay candidates. Names that differ only in the spelling of the corporate suffix ("Corp." and "Corporation," "Inc." and "Incorporated") are one name, not competitors. A record the web-address check or the age rule already rules out never counts as a competitor. Second, its age fits the vendor. A record formed more than five years before the vendor's stated founding year, or before the vendor's own web domain was registered, is not credited without one of the stronger details. Third, when the buyer supplied a web address, the address's root name must cover the record's name ("acmegov.com" covers "ACMEGOV, INC." and does not cover "ACME CONDUIT, LLC"). The web address is a consistency check on the name, never a tie. Records that fail these rules, and records under similar or contained names that nothing ties to the vendor, stay labeled CANDIDATE RECORDS: shown for your review, earning no credit and driving no warning. Names under four characters are credited only with one of the stronger details, whatever the record offers, because names that short collide even inside one state. A legal name that CONTAINS the vendor's name plus corporate boilerplate ("ZENCITY TECHNOLOGIES US, INC." for the brand "Zencity") is credited when a tie exists and the age and web-address checks pass; the reverse direction (a shorter record name inside the vendor's longer name) needs one of the stronger details. A record found through the research-to-registry name bridge is exempt from the web-address check, because the bridge exists to find a legal name the brand does not reveal. When a registry search returns several records under matching or contained names, every one of them is judged by these rules, and the lane is credited on the record that passes them best: a record one of the stronger details ties to the vendor comes first, then an exact-name record, then a contained name that a detail ties. Before version 1.8 each lane judged only its first exact-name record, so a vendor whose own record sat behind an untied namesake in the same registry was never credited there. When no record passes, the lane shows its first exact-name record as the candidate. When several registries credit a record, the identity sentence names the one a stronger detail ties, then the one whose legal name is the vendor's name plus a corporate suffix, then an exact-name record. The identity row's sentence and its green flag are written by code from the credited record. They never name a record the run did not credit.
**If adverse.** Weight: Critical, but only under a strict rule. "No registration found" can count against a vendor only when a definitive search actually ran and returned empty, in a place where the vendor itself claims to be registered. A registry record showing an affirmative end-of-registration designation ("Voluntarily Dissolved," "Revoked," "Forfeited") produces a record row and a finding ONLY when one of the stronger details above ties the record to this vendor. The designation is judged on the record that carries it: a namesake's ended registration listed beside the vendor's live record in the same registry never attaches to the vendor's record. An ended registration under a merely matching name that nothing connects to the vendor appears as a candidate record with a one-line question to the vendor, never as a finding. When the finding does arm, it is Critical for the entity's home-state registration and High otherwise, the note names the detail that ties the record to the vendor, and the report states only what the record shows, with the designation, the effective date, and a question about which legal entity would sign a contract today. A bare "Inactive" with no stated reason, a merger, or a routine withdrawal from one foreign state never arms this: a "Terminated," "Surrendered," or "Withdrawn" status on a registration OUTSIDE the entity's home state means the company ended its authority to do business in that one state, and the report shows it as record-only information ("no longer authorized in that state; ask which legal entity would sign a contract today"), never as a dissolution-class warning. The same words on the home-state registration are an affirmative end and keep full treatment. Absence from a registry we could not search is always "coverage limited," never a finding. Routine lapses like "annual report past due" are noted as Info, not treated as alarms; they are common at young companies. When nothing is found anywhere, the row's note is written by code, not by the AI: it leads with SEC EDGAR's national coverage of venture-funded companies, names the automated state registries that actually ran (and points at the honesty panel for any that were unreachable), and mentions EDGAR only when that search actually ran. The EDGAR check appears in the row's own source list whenever it ran, and the honesty panel states that the EDGAR search is national.
...
**How it runs.** Automatic lookup.
**If adverse.** Claimed federal customers with zero footprint in the federal payment record is weighted High. No federal footprint with no federal claims is neutral. Verified payments are a strong green flag, but only on an exact name match, only when the recipient's name is distinctive, and only when the record shows at least one award in the period searched. A recipient that merely resembles the vendor's name, an exact match on a name under four characters (which collides with unrelated companies), or a recipient entry with no awards appears as a labeled candidate row with the record linked, and it never earns a green flag or moves any credit. The credited row and its green flag keep the lookup's own caution that the recipient must be the same company. When the buyer supplied a web address, its root name must also cover the recipient's name, the same consistency check D1.1 applies to registry records. Another company's awards must not decorate this vendor's report.
...
Credit from every compliance feed in this dimension (and the cooperative list in D2.2) follows one attribution rule: an exact listing under a distinctive name is credited as before; a listing under a merely similar name, or an exact listing under a name under four characters, is credited only when the feed's own product entry names the vendor's product or web domain. The FedRAMP Marketplace feed adds one more path: a listing whose name begins with the vendor's own complete name is credited, because a curated feed listing "Tyler Technologies Data & Insights" for the vendor "Tyler Technologies" is that company's own entry. That path is deliberately narrow. It accepts only the vendor's company name, never a product name the pitch mentions; the name must have at least two words and not be a short, collision-prone name; and the listed name must start with the vendor's name in order. A listing that merely contains the vendor's words somewhere inside it, and the reverse direction (a shorter listed name inside the vendor's longer name), never credit. The credited FedRAMP row keeps its note to confirm at the link that the listed product is the one being pitched. Anything else appears as a labeled candidate row, visible, linked, and earning nothing, with a note to confirm the listing with the vendor.
...
The tool reads the vendor's own public website for every kind of submission, because most pitches leave it implicit and most readers do not know what to look for on it. What it reads: the homepage plus up to four high-value pages the homepage itself links (about, customers, security, and similar), fetched with strict size and time bounds. When the whole fetch fails and the run's clock allows, it is retried once from the start; a fetch that returned any pages is a result and is never re-run. When the reading of the fetched text fails, it is retried once on a shortened copy while the clock allows. Text hidden from human readers (invisible styling, oversized comments) is removed before any analysis sees it.
...
What a web address the buyer supplies can do: when the check runs from a web address, that address's root name acts as a consistency check on registry credit under D1.1. A registry record whose name the root does not cover is shown as a candidate record unless a stronger detail ties it to the vendor. This stops a product name that dominates a vendor's own page from crediting an unrelated company's registration. The address never adds names to the registry searches and is never itself a tie. A web address the tool found by search, rather than one the buyer typed, does not act as this check.

### Name-only submissions

A name-only check can carry the vendor's web address. When you type the address beside the name, the typed name still drives every registry search, and the address is treated the way a submitted web address is treated: its pages are read as the vendor's own site, its registration record counts toward identity, and its root name acts as the consistency check on registry credit described above. An address that is not a valid https site stops the check with a message instead of being ignored. The stored report records which address it checked and where that address came from.

A name-only check without a web address also says when the name collides. When it finds two or more live registry records under the exact name that nothing ties to the vendor, the honesty panel carries a "Name collision check" row under "Needs your attention", and the report overview repeats the notice. The count is a floor, because registry searches stop early on an exact match and cap their lists. The row is informational: the candidate records it points to already earn nothing and drive no warning, and the notice never changes the verdict.

When you give the tool only a company name, it first runs a short web search for the company's official website. The model only searches; our code picks the address, under strict rules (the address must match the vendor's name, and only actually-retrieved pages can nominate it). When the search fails for infrastructure reasons (nothing came back at all), it is retried once while the run's clock allows. When the search ran but did not surface the site, it is retried once with more specific queries: the company's name combined with words like "official website" or "company". If the second search does not surface it either, that is the answer. If a site is found, it is read as described above, and the site checks (domain age, mail configuration, web history, product infrastructure, engineering footprint) run against it. The report labels the address as matched by research. Its registration record can count toward identity verification only as the second identifier, only alongside a government registry record, and only when the site itself names the vendor; a company with nothing but a website can never resolve identity that way.

When the website step fails anyway, the report says so with a "Vendor website discovery" row in the honesty panel. When no site was found, the row says the website checks did not run. When a likely site was found but its pages could not be read, the row says the pages went unread and that the site's registration record could not count toward identity; the domain-record checks (registration date, mail setup, and similar) still run against the found address and appear in the panel on their own. In every case the row says this does not count against the vendor and that re-running the check with the vendor's web address will include the missing checks, and the report overview repeats the notice. When research later in the same run surfaces the site's address, the row says that instead, and the site's registration and mail checks appear in the panel even though its pages went unread. Before this disclosure existed, a failed website step was invisible: a report could land at "not enough to evaluate" with no explanation of the missing coverage.
...
**What the AI writes and what code writes.** Code writes every load-bearing sentence: the verdict tier and its rationale, every green flag, the identity row's sentence, the registry status rows, the candidate-record rows, the honesty panel, the coverage notes, and the identity row's note when no registration was found. A language model writes the verdict summary, the plain-language notes on the remaining rows (the other could-not-verify notes among them, each in the absence shape from Section 3 with the sources searched and the date), and the next steps, from the decided skeleton. Every sentence it writes is screened before publication. It may name only the vendor and the legal names of records credited to the vendor. A sentence that names any other company is dropped, and a surface with nothing left falls back to a fixed template. A summary sentence that says a state was searched must name a state whose registry lane ran. A second model reviews the wording of reports that carry adverse findings, contradictions, or adversarial-content findings, when that review completes within its time allowance. It may only tighten wording. It can never change the tier, never remove a High or Critical row, a contradicted row, an official-record row, or the identity row, and its replacement text passes the same screens. A review that does not complete changes nothing.
...
**An AI judgment can never assign this tier.** One trigger alone is not enough. A registry we could not search can never produce a trigger. Every trigger is listed in the report's rationale by the check that logged it, and the evidence link sits on that check's ledger row.
...
**Criteria (exact):** identity verified; no unresolved High or Critical findings; fewer than three dimensions carrying verified green flags. The report also states whether the vendor meets the **startup calibration bar** (Section 6): at least one of a GovRAMP listing at any level, a verified named customer, a federal award record, or an SEC filing, each on a record credited to the vendor. A SOC 2 report or a signed data protection agreement has no public registry, so neither can count toward the bar; the question pack asks for the security documents directly.
...
2. **The startup bar, applied automatically.** Code marks the *startup calibration bar* met when any one of these sits on a record credited to the vendor: a GovRAMP listing at any level, a verified named customer, a federal award record, or an SEC filing. Meeting it changes one sentence of the Tier 3 wording and nothing else. Artifacts with no public registry, such as a SOC 2 report or a penetration test summary, are requested through the question pack rather than counted. Demanding FedRAMP authorization or multiple simultaneous certifications from a seed-stage company is explicitly outside this methodology.
...
**Words the tool never uses about any named company or person:** scam, fraud, fraudulent, fake, sham, shell company (or "a shell" in any phrasing, or shell corporation, entity, firm, or operation), front company, lying, liar, deceptive, misleading, predatory, vaporware, snake oil, grift, illegitimate, bogus, con artist, criminal, dishonest, untrustworthy, and "high-risk vendor" as a label. Where a fact is adverse, the report states the fact, the source, and the date, and stops there.
...
- Every report records the **methodology version** that produced it (this document is version 1.8) and the **sector pack release** it drew questions from.
...
- **Bare-name checks and namesakes.** When a check runs from a company name alone, the tool may have no vendor-side fact to tell two same-name companies apart. If the only live registry record under that exact name belongs to a different company, the tool can credit it. Adding the vendor's web address beside the name closes most of this gap, and the collision notice marks names that several registered companies share. Registry searches also stop early on an exact match and cap their lists, so the count of same-name records the report gives is a floor, and a record the search never listed is never judged. New York's detail record, which carries the officer and address details that tie a record, is read for at most three exact-name records per check.
